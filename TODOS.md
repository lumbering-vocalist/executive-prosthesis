...
**What:** The CI step (`npx convex codegen` + `git diff --exit-code convex/_generated`) is in `.github/workflows/ci.yml`. It self-skips with a workflow notice until the `CONVEX_DEPLOY_KEY` repo secret exists — **founder action: create a deploy key in the Convex dashboard and add it as a repo secret.**
